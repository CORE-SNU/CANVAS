...
def main(goal_x, goal_y, num_iter, r_star):
    ...
    for times in range(num_iter):
        ...
        while not done:
            ...
            # --- (A) Trajectory error score function : predicted vs GT future ---
            score_traj = ScoreFunction(case="traj", return_type="mean")
            traj_err_mean = score_traj(
            ...
            # --- (B) Controller output error score function : pred-based vs GT-based plans ---
            # For GT(Oracle based) : no status update here, just for get controller input for GT
            velocity_gt, _, _, _, _, _, _ = controller(
                pos_x=position_x,
                pos_y=position_y,
                orientation_z=orientation_z,
                linear_x=linear_x,
            ...
            score_ctrl = ScoreFunction(case="control", return_type="mean")
            ctrl_err_mean = score_ctrl(
            ...
            print("Score_pred : ", traj_err_mean)
            print("Score_ctrl : ", ctrl_err_mean)
