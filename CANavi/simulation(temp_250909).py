...
def main(goal_x, goal_y, num_iter, r_star):
    ...
    # Save path for CI results
    results_dir_A = pathlib.Path("results_blockA")  # Directory for Block A
    results_dir_B = pathlib.Path("results_blockB")  # Directory for Block B
    ...
    for times in range(num_iter):
        ...
        # Per-iteration position CI records (predictor vs GT)
        iter_ciA_pos_records = []  # {'frame','pid','step','err','ci'}
        ...
        while not done:
            ...
            # Prediction accuracy CI @ steps {2,5,10}
            if isinstance(prediction_res, dict) and valid_obs_future_true:
                common_ids = set(prediction_res.keys()) & set(valid_obs_future_true.keys())
                for S in steps_to_eval:
                    ...
                    for pid in common_ids:
                        ...
                        if p.ndim == 2 and g.ndim == 2 and p.shape[1] >= 2 and g.shape[1] >= 2 \
                           and p.shape[0] > j and g.shape[0] > j:
                            perr = float(np.linalg.norm(p[j, :2] - g[j, :2]))
                            if np.isfinite(perr) and rstar > 0:
                                pci = (rstar - perr) / rstar
                                if np.isfinite(pci):
                                    iter_ciA_pos_records.append({
                                        'frame': frame,
                                        'pid': pid,
                                        'step': S,
                                        'err': perr,
        ...
        # Control-test CI buffers
        iter_ci_obj = []        # per-frame objective-diff CI
        iter_ci_cost = []       # per-frame component-cost-sum diff CI
        iter_ci_u_records = []  # per-frame, selected step input-diff CI, {'frame','step','err_u','ci_u'}
        # CSV records with frame
        iter_ci_obj_records  = []    # {'frame','err_obj','ci_obj'}
        iter_ci_cost_records = []    # {'frame','err_cost','ci_cost'}

        # Unified R* and steps for control-input CI
        rstar = r_star
        steps_to_eval = (2, 5, 10)

        # Predictor / controller / ACI
        data_dir = "/home/snowhan1021/tools_paper/CANavi/prediction/trajectron/models_17_Mar_2025_22_52_52lobby_data_ar3"
        obj_predictor = TrajectronPredictor(prediction_len=prediction_len, model_dir=data_dir, device='cpu')
        controller = GridMPC(n_steps=prediction_len, dt=dt)
        ...
        while not done:
            ...
            # --- Build valid history and GT futures (finite check & shape) ---
            observation = environment._get_obs()
            observation_future_true = environment._get_obs_future()
            valid_obs = {}
            valid_obs_future_true = {}
            ...
            # --- Simple collision check (distance to last history point) ---
            if valid_obs:
                dynamic_obs = valid_obs
                initial_positions = np.array([traj[-1, :2] for traj in dynamic_obs.values()])
                robot_pos = np.array([position_x, position_y])
            ...
            # =========================================================
            # (1) Predictor output at the current state
            # =========================================================
            pred_start = time.time()
            pred_out = obj_predictor(dynamic_obs if dynamic_obs else {})
            pred_time = time.time() - pred_start
            buffer_prediction_times_oracle.append(pred_time)

            # CP update
            ci_common = cp_module.update(dynamic_obs, pred_out if isinstance(pred_out, dict) else {})

            # =========================================================
            # (2) MPC with predictor (same robot state)
            # =========================================================
            vel_pred, info_pred, min_pred, inter_pred, term_pred, ctrl_pred, J_pred = controller(
                pos_x=position_x,
                pos_y=position_y,
                orientation_z=orientation_z,
                linear_x=linear_x,
                angular_z=angular_z,
                boxes=persistent_static_boxes,
                predictions=pred_out if isinstance(pred_out, dict) else {},
                confidence_intervals=ci_common,
                goal=goal
            )

            # =========================================================
            # (3) MPC with ground-truth futures (same robot state)
            # =========================================================
            vel_gt, info_gt, min_gt, inter_gt, term_gt, ctrl_gt, J_gt = controller(
                pos_x=position_x,
                pos_y=position_y,
                orientation_z=orientation_z,
                linear_x=linear_x,
                angular_z=angular_z,
                boxes=persistent_static_boxes,
                predictions=valid_obs_future_true if valid_obs_future_true else {},
                confidence_intervals=ci_common,
                goal=goal
            )

            # =========================================================
            # (4) Control-relevant errors & CIs
            # =========================================================
            # 4-1) Objective-difference CI
            err_obj = abs(J_pred - J_gt)
            if np.isfinite(err_obj) and rstar > 0:
                ci_obj = (rstar - err_obj) / rstar
                if np.isfinite(ci_obj):
                    iter_ci_obj.append(ci_obj)
                    iter_ci_obj_records.append({'frame': frame, 'err_obj': float(err_obj), 'ci_obj': float(ci_obj)})

            # 4-2) Cost-difference CI (approx: sum of components inter+term+ctrl)
            Jcomp_pred = float(inter_pred + term_pred + ctrl_pred)
            Jcomp_gt   = float(inter_gt   + term_gt   + ctrl_gt)
            err_cost = abs(Jcomp_pred - Jcomp_gt)
            if np.isfinite(err_cost) and rstar > 0:
                ci_cost = (rstar - err_cost) / rstar
                if np.isfinite(ci_cost):
                    iter_ci_cost.append(ci_cost)
                    iter_ci_cost_records.append({'frame': frame, 'err_cost': float(err_cost), 'ci_cost': float(ci_cost)})

            # 4-3) Control-input difference CI at selected steps {2,5,10}
            if (vel_pred is not None) and (vel_gt is not None):
                U_p = np.asarray(vel_pred, dtype=np.float64)
                U_g = np.asarray(vel_gt,   dtype=np.float64)
                if U_p.ndim == 2 and U_g.ndim == 2 and U_p.shape[1] >= 2 and U_g.shape[1] >= 2:
                    for S in steps_to_eval:
                        j = S - 1
                        if (U_p.shape[0] > j) and (U_g.shape[0] > j):
                            e = np.linalg.norm(U_p[j, :2] - U_g[j, :2])  # l2 norm on (v,w)
                            if np.isfinite(e) and rstar > 0:
                                ci_u = (rstar - e) / rstar
                                if np.isfinite(ci_u):
            ...
            # =========================================================
            # (5) Feasibility handling (use predictor feasibility)
            # =========================================================
            buffer_infeasibility.append(info_pred['feasible'])
            if not info_pred['feasible']:
                if infeasible_streak < len(buffer_vel):
                    v, w = buffer_vel[infeasible_streak]
                else:
                    v, w = 0.0, 0.0
            ...
            # =========================================================
            # (7) Advance environment once using predictor-based control
            # =========================================================
            cmd_linear_x, cmd_angular_z = vel_pred[0]
            robot_pose, done = environment.step([cmd_linear_x, cmd_angular_z])
            position_x, position_y, orientation_z = robot_pose
            print(frame, position_x, position_y, orientation_z, cmd_linear_x, cmd_angular_z, time.time() - detect_time)

            # Accumulate predictor-side costs for logging (consistent with Block A)
            minimum_cost.append(J_pred)
            buffer_intermediate_oracle.append(inter_pred)
            buffer_terminal_oracle.append(term_pred)
            buffer_control_oracle.append(ctrl_pred)

            frame += 1
            buffer_vel = vel_pred

        # ---- Iteration-level rates and summaries (unchanged) ----
        ...
        if iter_ci_obj:
            c = np.array(iter_ci_obj, dtype=np.float64); c = c[np.isfinite(c)]
            print(f"[Iter {times+1}][Control] CI_obj_mean={c.mean():.3f} (N={c.size})") if c.size else print(f"[Iter {times+1}][Control] CI_obj_mean: no finite")
        else:
            print(f"[Iter {times+1}][Control] CI_obj_mean: no records")

        if iter_ci_cost:
            c = np.array(iter_ci_cost, dtype=np.float64); c = c[np.isfinite(c)]
            print(f"[Iter {times+1}][Control] CI_cost_mean={c.mean():.3f} (N={c.size})") if c.size else print(f"[Iter {times+1}][Control] CI_cost_mean: no finite")
        else:
            print(f"[Iter {times+1}][Control] CI_cost_mean: no records")

        if iter_ci_u_records:
            cs = np.array([r['ci_u'] for r in iter_ci_u_records], dtype=np.float64); cs = cs[np.isfinite(cs)]
            if cs.size:
