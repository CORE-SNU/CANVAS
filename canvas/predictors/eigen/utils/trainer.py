...
class ETTrainer:
    ...
    def __init__(self, args, hyper_params):
        ...
        self.checkpoint_dir = hyper_params.checkpoint_dir + '/' + args.tag+'-STGCNN' + '/' + hyper_params.dataset + '/'
    ...
    def init_descriptor(self):
        ...
        obs_traj, pred_traj = augment_trajectory(obs_traj, pred_traj, flip=False, reverse=False)
...
class ETSTGCNNTrainer(ETSequencedMiniBatchTrainer):
    ...
    def __init__(self, base_model, model, hook_func, args, hyper_params):
        ...
        predictor_model = base_model(n_stgcnn=1, n_txpcnn=5, input_feat=1, output_feat=hyper_params.num_samples,
                                     kernel_size=3, seq_len=hyper_params.k+2, pred_seq_len=hyper_params.k).cpu()
        eigentraj_model = model(baseline_model=predictor_model, hook_func=hook_func, hyper_params=hyper_params).cpu()
