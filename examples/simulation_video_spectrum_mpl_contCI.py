...
def main(goal_x, goal_y, num_iter, r_star, dataset, predictor, video_fps, save_video,
    ...
    for times in range(num_iter):
        ...
        obj_predictor = Predictors(chosen_predictor=predictor, prediction_len=prediction_len, history_len=history_len,
                                   dt=dt, dataset=dataset, device='cpu')  # Trajectron++ predictor
        ...
        while not done:
            ...
            valid_obs, valid_obs_future_true = dynamic_observation_filter(observation, position_x, position_y,
                                                                          prediction_len, observation_future_true,
                                                                          max_ped)
            ...
            confidence_intervals = cp_module.update(valid_obs,
                                                    prediction_res if isinstance(prediction_res, dict) else {})
            confidence_intervals_gt = cp_module_gt.update(valid_obs,
                                                          valid_obs_future_true if isinstance(valid_obs_future_true,
                                                                                              dict) else {})
            ...
            if np.abs(position_x - goal[0]) < 0.3 and np.abs(position_y - goal[1]) < 0.3:
                ...
                buffer_avg_intermediate_cost.append(
                    np.sum(buffer_intermediate) / len(buffer_intermediate) if buffer_intermediate else np.nan)
                buffer_avg_terminal_cost.append(
                    np.sum(buffer_terminal) / len(buffer_terminal) if buffer_terminal else np.nan)
                buffer_avg_control_cost.append(
                    np.sum(buffer_control) / len(buffer_control) if buffer_control else np.nan)
                break
...
if __name__ == "__main__":
    ...
    main(args.goal_x, args.goal_y, args.num_iter, args.r_star, args.dataset, args.predictor, video_fps=args.video_fps,
         save_video=args.save_video,
         overlay=args.overlay, frame_offset=args.frame_offset, extracted_fps=args.extracted_fps,
         output_fps=args.output_fps, max_ped=args.max_ped, cont_CI=args.cont_CI, CI_t=args.CI_t)
